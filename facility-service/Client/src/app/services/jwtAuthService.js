...
import {navigationsUser, navigations} from "../navigations";
const config = {
  headers: {
  ...
  async loginWithUserNameAndPassword (username, password) {
    ...
    await this.getCurrentUser().then(res => {
      ...
      if (UserRoles.isAdmin()) {
        navigationStores = navigations;
      } else {
        navigationStores = navigationsUser;
      }
      localStorageService.setSessionItem("navigations", navigationStores);
    });
