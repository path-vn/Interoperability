...
class PreventOpportunisticInfections extends Component {
  state = {
  ...
  render() {
    ...
    let columns = [
      ...
      {
        title: t("patient.tab.testResult"),
        field: "testResultOther.display",
      },
      {
    ...
    return (
      <React.Fragment>
        <MaterialTable
          data={this.state.item ? this.state.item.drugResistanceList ? this.state.item.drugResistanceList : [] : []}//this.state.item ? this.state.item.cd4List ? this.state.item.drugResistanceList : [] : 
          columns={columns}
          options={{
            selection: false,
...
export default PreventOpportunisticInfections;
