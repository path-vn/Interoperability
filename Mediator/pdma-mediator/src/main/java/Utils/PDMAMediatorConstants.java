...
public class PDMAMediatorConstants {
	public static Valueset getOccupationByText(String value) {
		if (value != null && StringUtils.hasText(value)) {
			if (value.toLowerCase().contains("trẻ em") || value.toLowerCase().contains("Dưới 6 tuổi") || value.toLowerCase().contains("Under 6 years old")) {
				return HivEnums.OccupationValueset.CHILD.getData();
			} else if (value.toLowerCase().contains("học sinh") || value.toLowerCase().contains("sinh viên") || value.toLowerCase().contains("Học sinh") || value.toLowerCase().contains("Students")) {
	...
	public static Valueset getRapidTestResult(String value) {
		if (value != null && StringUtils.hasText(value)) {
			if (value.toLowerCase().contains("Nhiễm HIV đã lâu")) {
				return HivEnums.RapidTestValueset.LONG_TERM.getData();
			} else if (value.toLowerCase().contains("Mới nhiễm HIV")) {
	...
	public static Valueset getResultOther(String value) {
		if (value != null && StringUtils.hasText(value)) {
			if (value.toLowerCase().contains("không phát hiện") || value.toLowerCase().contains("kph - <200 bản sao/ml")) {
