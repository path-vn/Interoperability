...
import service.OpcAssistService;
...
public class DefaultOrchestrator extends UntypedActor {
	...
	OpcAssistService opcAssistService = new OpcAssistService();
...
//		System.out.println("BodyLengthDecode:"+decodedString.length());
		...
		List<OpcAssistDto> listOpcAssistDto = new ArrayList<OpcAssistDto>();
		AdapterObjectDto object = gson.fromJson(body, type);
//		System.out.println(object);
		if (object.getObjectType() == AdapterObjectType.SINGLE_DATA) {
			listOpcAssistDto.add(object.getSingleData());
		} else if (object.getObjectType() == AdapterObjectType.LIST_DATA) {
			listOpcAssistDto = object.getListData();
		}

		//// Check if ListData's size more than 10 QR, then split into parts of 10 QR
		if (listOpcAssistDto != null && listOpcAssistDto.size() > 0) {
			if (listOpcAssistDto.size() > splitSize) {
				if ((listOpcAssistDto.size() % splitSize) == 0) {
					int part = ((int) listOpcAssistDto.size() / splitSize);
					for (int i = 0; i < part; i++) {
						List<OpcAssistDto> subListOpcAssistDto = listOpcAssistDto.subList(splitSize * i,
								splitSize * (i + 1));
						Bundle questionnaireResponseBundle = opcAssistService
								.convertListOpcObjectToBundle(subListOpcAssistDto);
					...
					int part = ((int) listOpcAssistDto.size() / splitSize) + 1;
					for (int i = 0; i < part; i++) {
						List<OpcAssistDto> subListOpcAssistDto = new ArrayList<OpcAssistDto>();
						if (i < (part - 1)) {
							subListOpcAssistDto = listOpcAssistDto.subList(splitSize * i, splitSize * (i + 1));
						} else {
							subListOpcAssistDto = listOpcAssistDto.subList(splitSize * i, listOpcAssistDto.size());
						}
						Bundle questionnaireResponseBundle = opcAssistService
								.convertListOpcObjectToBundle(subListOpcAssistDto);
						String questionnaireResponseBody = parser.encodeResourceToString(questionnaireResponseBundle);
				...
				Bundle questionnaireResponseBundle = opcAssistService.convertListOpcObjectToBundle(listOpcAssistDto);
				String questionnaireResponseBody = parser.encodeResourceToString(questionnaireResponseBundle);
				if (IsUsingEncodeData) {
					questionnaireResponseBody = CoreUtils.encode(questionnaireResponseBody);
				}
