...
import com.globits.adapter.opcassist.domain.CaseOrg;
...
import com.globits.adapter.opcassist.types.PatientStatus;

public class OpcAssistDto implements Serializable {
...
//	private List<HivRiskFactorDto> listRiskPopulation;
	...
	public OpcAssistDto(Case caseEntity) {
		if (caseEntity != null) {
				...
				for (Location location : caseEntity.getPerson().getLocations()) {
					if (location.getAddressType().toString() == "CURRENT_ADDRESS") {
						...
						if (location.getDistrict() != null) {
							...
							this.currentDistrictCode = location.getDistrict().getCodeGso();
						}
